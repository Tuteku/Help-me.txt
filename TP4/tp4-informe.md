...
> Nota: en nuestro kernel (`6.17.0-29-generic`) los módulos del sistema vienen **comprimidos** (`des_generic.ko.zst`), por eso usamos el nombre del módulo y dejamos que `modinfo` resuelva la ruta y la descompresión. El comando del enunciado con `.ko` "pelado" no encuentra el archivo.
...
| `intree` | **No aparece** → es *out-of-tree* (esto es lo que *tainta* el kernel al cargarlo) | `Y` → forma parte del árbol oficial del kernel |
...
Conclusión: el módulo del sistema viene **firmado (SHA-512), dentro del árbol del kernel (`intree: Y`), con alias y dependencias declaradas y metadata completa**; el nuestro está fuera del árbol, sin firma y con lo mínimo. Esto explica directamente los mensajes de *taint* que vimos al hacer `insmod` en la sección anterior: el kernel marca a `mimodulo` como *out-of-tree* (le falta `intree: Y`) y *unsigned* (le faltan los campos `sig_*`). Los únicos campos que comparten valor son `license`, `vermagic` y `retpoline`.
